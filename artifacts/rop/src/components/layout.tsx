    ...
    <div className="min-h-[100dvh] w-full flex flex-col bg-background text-foreground overflow-x-hidden">
      ...
      <nav className="fixed bottom-0 left-0 right-0 h-16 bg-card/80 backdrop-blur-xl border-t border-border z-40">
        <div className="max-w-md mx-auto h-full flex items-center justify-around px-2">
          <NavItem href="/" icon={<Leaf size={20} />} label="Garden" active={pathname === "/"} />
          <NavItem href="/tasks" icon={<CheckSquare size={20} />} label="Tasks" active={pathname === "/tasks"} />
          <NavItem href="/collection" icon={<BookOpen size={20} />} label="Collection" active={pathname.startsWith("/collection") || pathname.startsWith("/codex")} />
          <NavItem href="/battle" icon={<Swords size={20} />} label="Battle" active={pathname === "/battle"} />
